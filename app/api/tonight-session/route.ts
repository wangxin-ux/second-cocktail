...
export async function POST(request: Request) {
  try {
    ...
    if (!signals) return NextResponse.json({ ok: false, error: "Invalid tonight signals" }, { status: 400 });
    const result = await createOrRefreshSession(signals, parseCookie(request.headers.get("cookie") ?? undefined));

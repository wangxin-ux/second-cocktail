...
const copy = {
  ...
  zh: {
    introEyebrow: "你的酒，由你决定", introTitle: "这杯酒可以停在这里，也可以成为一次介绍。", introBody: "Second 会用今晚的信号，找到一个真正有话题可以开始的人。", optional: "Connection 完全可选", mutual: "双方都必须接受", private: "不会自动分享联系方式", leave: "你随时可以离开", continue: "继续", consentTitle: "连接之前", consentBody: "请先了解边界，再允许这个虚构匹配演示使用今晚的信号。", consentLabel: "我理解这些边界，并同意在虚构匹配演示中使用浏览器档案。", start: "寻找一个人", demoCount: "演示可用人数 · {count} 个虚构档案", searching: "正在寻找一个真正有理由开口的人。", searchingBody: "我们在比较双方的酒和可选的今晚信号。你可以取消，Cocktail 不会丢失。", cancel: "取消", emptyTitle: "现在还没有其他人在这里。", emptyBody: "你的酒仍然属于你；当前演示场景里没有可用候选人。", keep: "保留我的酒", stay: "保持可匹配", staying: "你会在这个浏览器演示中保持可匹配。", retry: "稍后再试", candidate: "一次可能的介绍", why: "为什么是你们", opener: "可以这样开口", meet: "见面 5 分钟", pass: "跳过", block: "今晚不要再为我匹配这个人", waiting: "正在等待对方", waitingBody: "目前没有分享任何信息。只有对方也接受，才会继续。", leaveWaiting: "离开", mutualTitle: "你们都接受了。", mutualBody: "没有交换联系方式。现在只选择一个场所区域，进行五分钟的见面。", chooseArea: "选择见面区域", areaTitle: "在哪里见面？", areaBody: "只选择宽泛的场所区域，不填写精确座位或地址。", startFive: "开始 5 分钟", connection: "你们的五分钟 Connection", meetAt: "与 {name} 在{area}见面", end: "现在结束", help: "需要帮助？", helpTitle: "立即获得帮助", helpBody: "离开这次 Connection，并直接前往场所工作人员处。本演示不会替你联系工作人员。", leaveNow: "立即离开这次 Connection", findStaff: "直接前往场所工作人员处", report: "举报", ended: "这是你今晚的第二个故事。", endedBody: "无需做任何评价，也不会交换联系方式。", meetAnother: "认识另一个人", error: "Connection 暂时不可用。", errorBody: "你的 Cocktail 仍然保留。可以返回，或重新尝试这个演示。", back: "返回", reportTitle: "举报这次 Connection", reportBody: "请选择最接近的原因。本演示只在当前画面记录操作，无法把举报发送给场所工作人员或审核团队。", reportUnsafe: "我感到不安全", reportHarassment: "骚扰或不当行为", reportIdentity: "身份不符", reportOther: "其他", submitReport: "提交举报", reportResult: "仅为演示：举报没有被送达。真实上线时，举报必须送达场所工作人员或审核团队。", close: "关闭", sessionError: "需要先在 Tonight Session 中保存一杯 Cocktail，才能开始 Connection。", demo: "虚构浏览器演示",
  },
} as const;
    ...
    <main data-stage={stage} className="second-match relative min-h-dvh overflow-x-hidden bg-[#080808] px-5 pb-[max(2rem,env(safe-area-inset-bottom))] pt-[max(1.25rem,env(safe-area-inset-top))] sm:px-6">
      ...
      <div className="relative mx-auto w-full max-w-md">
        <header className="flex min-h-11 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3"><LanguageToggle /><EndTonightControl /><span className="text-[0.56rem] font-semibold uppercase tracking-[0.25em] text-amber-100/45">second / connection</span></div>
        </header>
        ...
        <section className="second-stage flex min-h-[calc(100svh-5rem)] flex-col justify-center py-8">
          {stage === "intro" && cocktail ? <>
            ...
            <div className="mt-7 border-y border-white/[0.14] py-5"><p className="second-micro">{language === "zh" ? "今晚的 Cocktail" : "Tonight’s cocktail"}</p><p className="mt-3 text-xl font-medium tracking-[-0.035em] text-stone-100">{cocktail.name}</p></div>
          ...
          {stage === "consent" ? <>
            ...
            <ul className="mt-5 grid gap-2 text-xs leading-5 text-white/52"><li>✓ {c.mutual}</li><li>✓ {language === "zh" ? "只有双方接受后才会显示见面区域" : "A meeting area appears only after both accept"}</li><li>✓ {c.private}</li><li>✓ {c.leave}</li><li>✓ {language === "zh" ? "离开后，你的 Cocktail 仍会保留" : "Your cocktail stays when you leave"}</li></ul>
          ...
          {stage === "ended" ? <><p className="text-[0.6rem] font-semibold uppercase tracking-[0.32em] text-amber-100/48">second / connection</p><h1 className="mt-4 text-[3rem] font-medium leading-none tracking-[-0.06em] text-stone-100">{c.ended}</h1><p className="mt-5 text-sm leading-6 text-white/48">{c.endedBody}</p><div className="mt-8 grid gap-3"><Link href={drinkHref} className={primaryButton}>{c.keep}</Link><button type="button" className={secondaryButton} onClick={meetSomeoneElse}>{c.meetAnother}</button></div></> : null}

...
export default function RealtimeMatchExperience({ spirit, flavor }: { spirit: { id: SpiritId; name: string }; flavor: { id: FlavorId; name: string } }) {
  ...
  const returnFromEndedMatch = () => {
    ...
    void service.dismissEndedMatch(pairId)
      ...
      .catch((reason) => {
        setError(reason instanceof Error ? reason.message : c("Unable to return to your drink.", "暂时无法回到我的酒。"));
  ...
  const restartFromEndedMatch = () => {
    ...
    void service.dismissEndedMatch(pairId)
      ...
      .catch((reason) => setError(reason instanceof Error ? reason.message : c("Unable to start again.", "暂时无法重新开始。")));
  };

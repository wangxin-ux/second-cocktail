...
const signals = (profile: ReturnType<typeof useSecondProfile>["profile"], cocktail: ReturnType<typeof readTonightCocktailSession>, spirit: SpiritId, flavor: FlavorId, language: "en" | "zh", directMatch: boolean) => ({ nickname: profile.nickname ?? "", age: profile.age ?? 0, ...(profile.heightCm ? { heightCm: profile.heightCm } : {}), ...(profile.gender ? { gender: profile.gender } : {}), ...(profile.preferredGender ? { preferredGender: profile.preferredGender } : {}), ...(profile.minPartnerHeightCm ? { minPartnerHeightCm: profile.minPartnerHeightCm } : {}), meetingLocation: profile.meetingLocation ?? "", energy: profile.energy ?? "open", ...(profile.mbti ? { mbti: profile.mbti } : {}), spirit: cocktail?.spirit ?? spirit, flavor: cocktail?.flavor ?? flavor, cocktailId: cocktail?.result.recipe.id ?? (directMatch ? "direct-profile-match" : ""), cocktailName: cocktail ? localizeCocktailRecipe(cocktail.result.recipe, language).name : (directMatch ? "second" : ""), ageBand: 0 });

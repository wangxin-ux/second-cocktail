...
export default function PrivacySummary({ className = "" }: { className?: string }) {
  ...
  const text = language === "zh" ? {
    ...
    items: [
      ["保存在哪里？", "Tonight 数据仅保存在这台设备的当前浏览器会话中。"],
      ["什么会参与匹配？", "年龄用于参与资格；今晚状态、MBTI、星座与 Cocktail 信号可用于解释匹配。"],
      ["对方何时能看到？", "只有你主动开始 Connection 后，对方才可能看到允许公开的预览；见面区域必须等双方接受。"],
      ["会分享联系方式吗？", "不会。Second 不会自动分享电话、社交账号或精确位置。"],
      ["如何删除？", "选择“结束今晚”会清除档案、Cocktail、匹配、屏蔽名单和见面区域；语言偏好保留。"],
    ],
  } : {
  ...
  return <>
    ...
    {open ? <div className="fixed inset-0 z-[70] flex items-end bg-black/75 p-5 sm:items-center sm:justify-center" role="dialog" aria-modal="true" aria-labelledby="privacy-title">
      <div className="second-dialog">
        <p className="second-kicker">Privacy / Tonight only</p><h2 id="privacy-title" className="second-section-title mt-3 text-stone-100">{text.title}</h2>

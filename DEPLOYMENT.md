...
4. **Environment.** Copy `.env.example` to `.env.production`, restrict permissions, and set real `DATABASE_URL`, `APP_ORIGIN`, `VENUE_MEETING_AREAS`, and `NEXT_PUBLIC_MATCH_MODE=realtime`.
...
11. **HTTPS.** Configure the actual domain/certificate. `APP_ORIGIN` must exactly match the public HTTPS origin.
